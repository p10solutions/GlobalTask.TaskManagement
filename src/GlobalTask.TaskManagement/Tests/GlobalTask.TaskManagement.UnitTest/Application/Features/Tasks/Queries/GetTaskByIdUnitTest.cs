...
{
    public class GetTaskByIdUnitTest
    {
       readonly Mock<ITaskRepository> _taskRepository;
       readonly Mock<ILogger<GetTaskByIdHandler>> _logger;
       readonly Mock<INotificationsHandler> _notificationsHandler;
       readonly Fixture _fixture;
       readonly GetTaskByIdHandler _handler;

        public GetTaskByIdUnitTest()
        {
            _taskRepository = new Mock<ITaskRepository>();
            _logger = new Mock<ILogger<GetTaskByIdHandler>>();
            _notificationsHandler = new Mock<INotificationsHandler>();
            _fixture = new Fixture();
            _handler = new GetTaskByIdHandler(_taskRepository.Object, _logger.Object, _notificationsHandler.Object);
        }

        [Fact]
        public async Task Task_Should_Be_Geted_Successfully_When_All_Information_Has_Been_Submitted()
        {
            var taskQuery = _fixture.Create<GetTaskByIdQuery>();
            var task = _fixture.Create<TaskItem>();
            _taskRepository.Setup(x => x.GetAsync(It.IsAny<Guid>())).ReturnsAsync(task);

            var response = await _handler.Handle(taskQuery, CancellationToken.None);

            Assert.NotNull(response);
            _taskRepository.Verify(x => x.GetAsync(It.IsAny<Guid>()), Times.Once);
        }
        ...
        {
            var taskQuery = _fixture.Create<GetTaskByIdQuery>();
            _taskRepository.Setup(x => x.GetAsync(It.IsAny<Guid>())).Throws(new Exception());
